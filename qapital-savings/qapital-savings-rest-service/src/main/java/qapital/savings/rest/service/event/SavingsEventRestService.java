...
public class SavingsEventRestService {
    ...
    @GetMapping("/trigger/{transactionId}/{userId}")
    public ResponseEntity<SavingsEvent> getSavingsEventForTransaction(@PathVariable("transactionId") Long transactionId, @PathVariable("userId") Long userId) {
        if (!isNull(userId) && !isNull(transactionId)) {
            LOGGER.info("Attempt to fetch savingsEvent generated by transaction {} for user {} ", transactionId, userId);
            SavingsEvent savingsEvent = savingsEventService.getSavingsEventForTransaction(transactionId, userId);
            return ResponseEntity.ok(savingsEvent);
        }
        return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
    }
